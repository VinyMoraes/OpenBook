...
if (isset($_GET['alugado']) && $_GET['alugado'] === 'true' && isset($_GET['livro_id'])) {
    ...
    if ($stmt->execute()) {
        // Mensagem de sucesso
        echo "<script>alert('O livro foi alugado com sucesso!'); window.location.href='../painelUser/alugarLivro.php';</script>";
    } else {
        // Mensagem de erro
        echo "<script>alert('Erro ao alugar o livro: " . $conexao->error . "'); window.location.href='../painelUser/alugarLivro.php';</script>";
    }
